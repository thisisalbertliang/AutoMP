...
def plot_emb_runtime_vs_vocab_size():
    for phase in PHASES:
        ...
        plt.title(f'Embedding Layer {phase_title}: Average Elasped Time per Pass v.s. Vocab Size (Using {NUM_GPUS[-1]} GPU)')
...
def plot_emb_runtime_vs_num_gpus():
    for phase in PHASES:
        ...
        phase_title = phase.get_pretty_name()
        plt.title(f'Embedding Layer {phase_title}: Average Elasped Time per Pass v.s. Number of GPUs')
        plt.xlabel('Number of GPUs')
...
def plot_gpt2_runtime_vs_num_params():
    for phase in PHASES:
        ...
        phase_title = phase.get_pretty_name()
        plt.title(f'GPT2 {phase_title}:\nAverage Elasped Time per Pass v.s. Number of Learnable Parameters (Using {NUM_GPUS[-1]} GPU)')
