...
class ParallelMLP(torch.nn.Module):
    ...
    def __init__(self, num_features, num_classes, hidden_sizes=[20480,20480,20480]):
        ...
        self.fc1_relu = ColumnParallelLinear(input_size=num_features, output_size=hidden_sizes[0], gather=True)
        self.fc2_relu = ColumnParallelLinear(input_size=hidden_sizes[0], output_size=hidden_sizes[1], gather=True)
        # self.fc3_relu = ParallelLinear(input_size=hidden_sizes[1], output_size=hidden_sizes[2], gather=True)
        self.fc_final = ColumnParallelLinear(input_size=hidden_sizes[1], output_size=num_classes, gather=True)
